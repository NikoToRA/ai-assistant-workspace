...
```bash
# 動画メタデータ取得
skills/youtube-notes/.venv/bin/yt-dlp -J "[YouTube URL]"
```
...
skills/youtube-notes/.venv/bin/youtube_transcript_api [VIDEO_ID] --languages ja en --format text
```
...
1. 音声を抽出
   ```bash
   skills/youtube-notes/.venv/bin/yt-dlp -x --audio-format mp3 -o "audio.mp3" "[YouTube URL]"
   ```
