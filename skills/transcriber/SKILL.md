---
name: transcriber
description: 音声ファイルをテキストに文字起こしするスキル。mp3/wav/m4a/ogg/flac形式に対応。faster-whisperを使用し、tiny/base/small/medium/large-v3の5つのモデルから精度と速度のバランスを選択可能。長時間音声はバックグラウンド実行に対応。「文字起こしして」「音声をテキストに変換して」で使用。
---
...
cd skills/transcriber && .venv/bin/python transcribe.py "[音声ファイルパス]" --model-size [モデル] --output "[出力パス].txt"

# 長い音声（10分以上）はバックグラウンド
nohup skills/transcriber/.venv/bin/python skills/transcriber/transcribe.py "[音声ファイルパス]" --model-size [モデル] --output "[出力パス].txt" > /tmp/transcription.log 2>&1 &
echo "PID: $!"
```
